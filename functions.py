...
#-------------------------------------------------------------------------------
# train model
def train_epoch(gcn_net,tr_net, train_loader, criterion, optimizer,optimizer2,indexs_train,band_patch):
    objs = AvgrageMeter()
    top1 = AvgrageMeter()
    tar = np.array([])
    pre = np.array([])
    for batch_idx, (A,batch_data, batch_target) in enumerate(train_loader):
        ...
        gcn_pred = gcn_net(batch_data,batch_A,indexs_train,band_patch)
...
#-------------------------------------------------------------------------------
# validate model
def valid_epoch(gcn_net,tr_net, valid_loader, criterion,indexs_test,band_patch):
    objs = AvgrageMeter()
    top1 = AvgrageMeter()
    tar = np.array([])
    pre = np.array([])
    for batch_idx, (A,batch_data, batch_target) in enumerate(valid_loader):
        ...
        gcn_pred = gcn_net(batch_data,batch_A,indexs_test,band_patch)
        batch_pred = tr_net(gcn_pred)
        loss = criterion(batch_pred, batch_target)
